    ...
    for sentence in sentences:
        ...
        if len(words) > 2:
            for i, _ in enumerate(words):
                if i < len(words) - 2:
                    dual_words_set.add((words[i], words[i+1], words[i+2]))
    ...
    for sentence in sentences:
        ...
        if len(splitted) > 3:
            for i, word in enumerate(splitted):
                if i == 0:
                    if 'BEGIN' in matrix[(word, splitted[i+1], splitted[i+2])]:
                        matrix[(word, splitted[i+1], splitted[i+2])]['BEGIN'] += 1
                    else:
                        matrix[(word, splitted[i+1], splitted[i+2])]['BEGIN'] = 1

                if i == len(splitted) - 3:
                    if 'END' in matrix[(word, splitted[i+1], splitted[i+2])]:
                        matrix[(word, splitted[i+1], splitted[i+2])]['END'] += 1
                    else:
                        matrix[(word, splitted[i+1], splitted[i+2])]['END'] = 1

                if i < len(splitted) - 3:
                    if splitted[i+2] in matrix[(word, splitted[i+1], splitted[i+2])]:
                        matrix[(word, splitted[i+1], splitted[i+2])][splitted[i+3]] += 1
                    else:
                        matrix[(word, splitted[i+1], splitted[i+2])][splitted[i+3]] = 1
